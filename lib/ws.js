  ...
  wss.on("connection", (ws) => {
    ws.on("message", async (raw) => {
      try {
        ...
        /**
         * SESSION CHANGED (group created / member added/removed / title updated)
         * client kirim setelah berhasil PATCH/POST sesi grup.
         * Server fetch ulang detail sesi lalu broadcast ke semua participantIds
         * (termasuk yang baru di-add, dan -- bila ada -- yang baru di-remove
         * supaya UI mereka bisa membersihkan sesi).
         */
        if (data.type === "session-changed") {
          const { sessionId, removedUserIds } = data;
          if (!sessionId) return;
          ...
          const participantIds = sessionDetail.participantIds || [];
          const notifyIds = Array.from(
            new Set([
              ...participantIds,
              ...(Array.isArray(removedUserIds) ? removedUserIds : []),
            ]),
          );

          broadcastToParticipants(
            { type: "session-changed", session: sessionDetail },
            notifyIds,
          );
          return;
        }
