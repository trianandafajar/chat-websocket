    ...
    ws.on("message", async (raw) => {
      try {
        ...
        if (data.type === "auth") {
          ...
          fetch("http://localhost:3042/api/presence", {
            method: "POST",
        ...
        if (data.type === "message") {
           ...
           */
          const res = await fetch("http://localhost:3042/api/messages", {
            method: "POST",
            headers: {
           ...
           */
          const sessionRes = await fetch(`http://localhost:3042/api/sessions/${sessionId}`, {
            headers: {
              "Content-Type": "application/json",
            },
          });
